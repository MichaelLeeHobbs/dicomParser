...
| 6 — Release + dcmtk.js swap        | **IN PROGRESS** (RCs published) | `2.0.0-rc.1` and `2.0.0-rc.2` published to npm under the `rc` dist-tag (Trusted Publishing configured, provenance, prerelease GitHub Releases); alpha `2.0.0-alpha.0` deprecated. rc.2 rolls up parseHeadAsync (#63), BulkRange vr/encapsulated (#65), and `/compat` readPart10Header (#62). Remaining blockers (docs/release-runbook.md): dcmtk.js swap + 198-file DCMTK differential + d-dart soak (cross-repo/user-gated), then `2.0.0` final repoints `latest` off the alpha.                                                                                                                                                                                                                                                                            |
...
- [x] Provenance badge visible on npm after first CI-driven publish (rc.1/rc.2 published with `--provenance`)

...
| 6 — Release + dcmtk.js swap        | **IN PROGRESS** (RCs published) | `2.0.0-rc.1`/`rc.2`/`rc.3` published to npm under the `rc` dist-tag (Trusted Publishing configured, provenance, prerelease GitHub Releases); alpha `2.0.0-alpha.0` deprecated. rc.2 rolled up parseHeadAsync (#63), BulkRange vr/encapsulated (#65), and `/compat` readPart10Header (#62); rc.3 the strict encapsulated head-read fragment hop (#67, found in dcmtk.js adoption). Remaining blockers (docs/release-runbook.md): dcmtk.js swap + 198-file DCMTK differential + d-dart soak (cross-repo/user-gated), then `2.0.0` final repoints `latest` off the alpha.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| 2.1 — Field-review roadmap         | **IN PROGRESS**                 | `master` is the 2.1 line (`2.1.0-alpha.0`) while the 2.0 rc soaks downstream; 2.0.0 final cuts from the `v2.0.0-rc.3` tag. Order: P0s #34 (typed `needMoreBytes` truncation outcome — `parsePartial`, landed) → #33 (push parser `PushParser`, landed; incremental-inflate signals for deflated input deferred) → #35 (resolved-tag-set stops, landed — all three P0s done), then P1s #36 (`/dictionary` subpath, landed) → #37 (`toDicomJson`, landed; JSON→dataset direction deferred), #38 (lazy frame index + EOT, landed) — all P1s complete, #39 (FMI knobs, landed), #40 (rawBytesCopy + view docs, landed), #56 (PN ^/= charset resets, landed) — P1s complete except #38. Test infra #45 (nightly fuzz at FUZZ_SCALE + persisted crash-regression corpus) landed; #44 (corpus diversification) needs real vendor fixtures, so it stays user-owned. P2s #41 (streaming/into-buffer write), #42 (nested/predicate edits) and #43 (non-conformant fixture knobs) all landed — every 2.1 item is in except #44 (corpus diversification), which needs real vendor fixtures and stays user-owned. **Do not publish a `2.1.0-rc.*` until 2.0.0 final ships** — publish.yml derives the dist-tag from the version suffix, so it would repoint the `rc` tag the soak consumes. |
...
| 6     | Release `2.0.0-rc.x` → swap into dcmtk.js behind the existing `engine` option → soak → `2.0.0` final                               | d-dart soak clean                                                                     |
...
- [x] Provenance badge visible on npm after first CI-driven publish (rc.1/rc.2/rc.3 published with `--provenance`)
- [ ] `latest` dist-tag correct after 2.0.0 final (overwrites the forced alpha `latest`). **Interim (field review D4, done):** `2.0.0-alpha.0` deprecated (`npm deprecate … "prerelease — pin explicitly"`); RCs publish under the `rc` dist-tag, so `latest` stays on the alpha until the `2.0.0` final repoints it (never at an rc).
...
**D4 (release, done):** alpha `2.0.0-alpha.0` deprecated; `latest` repoints at the `2.0.0`
final (not an rc) — tracked in §9 → Release engineering.
